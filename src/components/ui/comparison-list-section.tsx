			...
			<AnimatePresence>
				{isWithout && (
					...
					>
						<button
							onClick={() => setActiveTab("with")}
							className={`h-11 px-6 w-full rounded-2xl border ${accent.border} ${accent.bgLight} ${accent.text} font-display font-bold text-base inline-flex items-center justify-center ${accentColor === "ori-message" ? "shadow-blue-btn-soft" : "shadow-orange-btn-soft"}`}
						>
							{ctaLabel}
						</button>
					</motion.div>
